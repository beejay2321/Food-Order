...
def _distance_between_locations(lat1, lon1, lat2, lon2):
    ...
    lon1 = math.radians(int(lon1))
    lat2 = math.radians(int(lat2))
    lon2 = math.radians(int(lon2))
...
def order_made():
    ...
    # calculate fee
    lat1, lon1 = db.cursor().execute("select U_ys, U_age from Users where UID = ?",
                                     (UID, )).fetchone()
    lat2, lon2 = db.cursor().execute("select S_stall, S_item from Stores where S_owner = ?",
...
def register():
    ...
    if len(phonenumber) != 11 or not phonenumber.isdigit():
        flash("Please check: phone number can only contain 11 digits")
        return redirect(url_for("sign_up"))
    ...
    try:
        ...
        flash("Please check: year level or stall number can only be number")
        return redirect(url_for("sign_up"))
    if not (-90 <= number <= 90 and -180 <= age <= 180):
        flash("Please check your year level or stall number")
        return redirect(url_for("sign_up"))
    ...
    try:
        db.cursor().execute('''
            insert into Users (U_account, U_password, U_name, U_type, U_ys, U_age, U_phone, U_balance)
            values (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (Account, password, name, 0, number, age, phonenumber, 0))
...
def search_shops():
    ...
    rst = db.cursor().execute(
        ...
        with dis(SID, gio_dis) as (
                select SID, _GIO_DIS(S_stall, S_item, :U_lat, :U_lon) as gio_dis
                from Stores
            ),
            dis_cat(SID, distance) as (
                select SID, case
                    when gio_dis >= :far then 'Alangilan Cafeteria'
                    when gio_dis >= :medium then 'Alangilan Cafeteria'
                    else 'Alangilan Cafeteria'
                end as distance
    ...
    # instr(a, b) > 0 means if a contains substring b
    # yearlevel/stall number and age are checked, ordering is a list(user can only select), so don't worry about SQL injection
    table = {'tableRow': []}
...
def edit_information():
    ...
    # check validity
    try:
        number, age = int(number), int(age)
    except ValueError:
        flash("Please check your year level or stall number can only be integer")
        return redirect(url_for("nav"))

    if not (-90 <= int(number) <= 90 and -180 <= int(age) <= 180):
        flash("Please check your year level or stall number")
        return redirect(url_for("nav"))
...
def shop_register():
    ...
    shop_stall = request.form['shop_stall']
    shop_item = request.form['shop_item']
    ...
    # stall and item
    try:
        number = int(shop_stall)
        item = int(shop_item)
    except ValueError:
        flash("Please check: locations can only be int")
        return redirect(url_for("nav"))

    if not (-90 <= number <= 90 and -180 <= item <= 180):
    ...
    try:
        shop_info = db.cursor().execute('''
            insert into Stores (S_name, S_stall, S_item, S_phone, S_foodtype, S_owner)
            values (?, ?, ?, ?, ?, ?)
        ''', (shop_name, number, item, owner_phone, shop_category, UID))
        # print(shop_name, number, item, owner_phone, shop_category, UID)
    except sqlite3.IntegrityError:
